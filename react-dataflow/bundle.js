(function () {
	...
	*/
	...
	const importMeta = {
	  url: new URL('./node_modules/@d3/brushable-scatterplot/589f9a25112ad1c2@213.js', (document.currentScript && document.currentScript.src || new URL('bundle.js', document.baseURI).href)).href
	};
	// https://observablehq.com/@d3/brushable-scatterplot@213
	function define$1(runtime, observer) {
	  const main = runtime.module();
	  const fileAttachments = new Map([["cars-2.csv", new URL("./files/53c407ee531bab128477148c9e28c49dd06bf83a93ae317e58dbb9fc819db0d4f6c4fb9646fa2fe20faad76addee20cfc360eab2362eeaec3340a5e4655b9996", importMeta.url)]]);
	  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
	  main.variable(observer()).define(["md", "data"], function (md, data) {
	    return md`# Brushable Scatterplot
...
This chart shows the inverse relationship between engine power (*y*-axis) and fuel efficiency (*x*-axis) in ${data.length} cars from 1970–1982. Brushing this scatterplot will show the selected data points.`;
	  ...
	  main.variable(observer("viewof selection")).define("viewof selection", ["d3", "width", "height", "xAxis", "yAxis", "data", "x", "y"], function (d3, width, height, xAxis, yAxis, data, x, y) {
	    ...
	    const dot = svg.append("g").attr("fill", "none").attr("stroke", "steelblue").attr("stroke-width", 1.5).selectAll("circle").data(data).join("circle").attr("transform", d => `translate(${x(d.x)},${y(d.y)})`).attr("r", 3);
	    svg.call(brush);
	    ...
	    function brushed({
	      ...
	      if (selection) {
	        ...
	        value = dot.style("stroke", "gray").filter(d => x0 <= x(d.x) && x(d.x) < x1 && y0 <= y(d.y) && y(d.y) < y1).style("stroke", "steelblue").data();
	      } else {
	  ...
	  main.variable(observer("x")).define("x", ["d3", "data", "margin", "width"], function (d3, data, margin, width) {
	    return d3.scaleLinear().domain(d3.extent(data, d => d.x)).nice().range([margin.left, width - margin.right]);
	  });
	  main.variable(observer("y")).define("y", ["d3", "data", "height", "margin"], function (d3, data, height, margin) {
	    return d3.scaleLinear().domain(d3.extent(data, d => d.y)).nice().range([height - margin.bottom, margin.top]);
	  });
	  main.variable(observer("xAxis")).define("xAxis", ["height", "margin", "d3", "x", "width", "data"], function (height, margin, d3, x, width, data) {
	    return g => g.attr("transform", `translate(0,${height - margin.bottom})`).call(d3.axisBottom(x)).call(g => g.select(".domain").remove()).call(g => g.append("text").attr("x", width - margin.right).attr("y", -4).attr("fill", "#000").attr("font-weight", "bold").attr("text-anchor", "end").text(data.x));
	  });
	  main.variable(observer("yAxis")).define("yAxis", ["margin", "d3", "y", "data"], function (margin, d3, y, data) {
	    return g => g.attr("transform", `translate(${margin.left},0)`).call(d3.axisLeft(y)).call(g => g.select(".domain").remove()).call(g => g.select(".tick:last-of-type text").clone().attr("x", 4).attr("text-anchor", "start").attr("font-weight", "bold").text(data.y));
	  });
	  main.variable(observer("data")).define("data", ["d3", "FileAttachment"], async function (d3, FileAttachment) {
	    return Object.assign(d3.csvParse(await FileAttachment("cars-2.csv").text(), ({
	      Name: name,
	      Miles_per_Gallon: x,
	      Horsepower: y
	    }) => ({
...
Thanks to [John Alexis Guerra Gómez](/@john-guerra) for suggestions.`;
	...
	function BrushableScatterplot(_ref) {
	  ...
	  react.useEffect(function () {
	    var newModule = new Runtime().module(define$1, function (name) {
	      if (name === "viewof selection") return Inspector.into(ref.current.querySelector(".viewof-selection"))(); // passes selection back up to parent component
	  ...
	  return /*#__PURE__*/react.createElement("div", {
	    className: "BrushableScatterplot",
	    ref: ref
	  }, /*#__PURE__*/react.createElement("div", {
	    className: "viewof-selection"
	  }), /*#__PURE__*/react.createElement("p", null, "Credit: ", /*#__PURE__*/react.createElement("a", {
	    href: "https://observablehq.com/@d3/brushable-scatterplot"
	  }, "Brushable Scatterplot by D3")));
	}

	var App = function App() {
	  // passed to notebook
	  var _useState = react.useState(600),
	      _useState2 = _slicedToArray(_useState, 2),
	      height = _useState2[0],
	      setHeight = _useState2[1]; // setter passed to notebook so it can set and this component can render
	  ...
	  }, /*#__PURE__*/react.createElement("h2", null, "Observable notebook"), /*#__PURE__*/react.createElement(BrushableScatterplot, {
	    height: height,
	  ...
	  }, /*#__PURE__*/react.createElement("h2", null, "React component"), /*#__PURE__*/react.createElement("p", null, "Height of chart: ", /*#__PURE__*/react.createElement("input", {
	    type: "number",
	    value: height,
	    onChange: function onChange(e) {
	      return setHeight(e.target.value);
	    }
	  })), /*#__PURE__*/react.createElement("p", null, "Current selection: ", selection.length ? selection.map(function (d) {
	    return d.name;
	  }).join(", ") : "empty")));
	};
